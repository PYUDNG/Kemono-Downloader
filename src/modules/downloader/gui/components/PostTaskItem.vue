...
import { appTaskDetailInjectionKey, providerInjectionKey } from '../utils.js';
...
const appTaskDetail = inject(appTaskDetailInjectionKey)!;
/**
 * 用户查看任务详情
 * @param _e 点击事件
 * @param task 任务实例，和props传入的task应当相同
...
function detail(_e: PointerEvent, task: IPostDownloadTask) {
    appTaskDetail.showWithTasks(task.fileTasks);
}
</script>
...
<template>
    ...
    >
        ...
        <template #title>
            {{ task.data ? task.name : t(tsCommonPrefix + 'title-nodata') }}
        </template>
