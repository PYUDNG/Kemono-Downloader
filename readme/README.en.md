...
A modern Kemono downloader user script, providing a beautiful UI interface, multiple downloaders, and highly customizable features.
...
- Development server supports HTTPS
...
After the build is complete, the `kemono-downloader.(min|greasyfork)?.user.js` file will be generated in the project's `/dist/` directory. You can install it by following these steps:
...
- Paste the content of the user script generated by the build
