...
A modern Kemono downloader user script, providing a beautiful UI interface, multiple downloader options, and highly customizable features.
...
The production build artifacts will be created in `/dist/`
...
The above commit message is for example only. In practice, for such a large number of updates, try to split them across multiple commits.
